...
1. 根據作業系統執行對應指令，取得「美投講美股」最新影片的標題與 URL：
   - **Mac**: `uv run skills/get_latest_video.py "https://www.youtube.com/@MeiTouJun/videos"` (⚠️ **極度重要**：請務必使用 `/videos` 結尾，**絕對不可**像小翠頻道一樣使用 `/streams`，因為美投頻道沒有 streams 分頁)
   - **Windows**: 依環境設定使用容器指令（如 `& $env:CONTAINER_RUNTIME exec $env:CUI_CONTAINER uv run skills/get_latest_video.py "https://www.youtube.com/@MeiTouJun/videos"`）或直接執行（`uv run skills/get_latest_video.py "https://www.youtube.com/@MeiTouJun/videos"`） (⚠️ **極度重要**：同樣必須使用 `/videos` 結尾)
