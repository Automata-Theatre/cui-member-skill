...
#### Step 2: 檢查是否已下載與處理
1. 從剛才取得的 **URL** 中提取出 YouTube 影片 ID（例如 `https://www.youtube.com/watch?v=9tREtYASGbs` 的影片 ID 為 `9tREtYASGbs`）。
2. 使用命令 `tail -n 20 logs/download.log` 讀取最近的下載紀錄，檢查該 **影片 ID** 是否已存在於紀錄中。
3. 判斷邏輯：
   - 若該影片 ID 已存在於 `logs/download.log` 之中，代表已經處理過。請向使用者回報「美投講美股：最新影片已處理過，無須重新下載」，並 **結束此任務**。
...
5. **結束任務**: 若你是單獨執行 `/scan_meitou_stock`，請到此（完成 `/summarize`）為止，向使用者回報「美投講美股：新影片已下載並完成摘要」，**不要**接著執行 `/scan_cui`、`/scan_meitou_news`、`/compare`、`/sync` 或 `/archive`。
